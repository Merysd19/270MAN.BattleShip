...
int torpedo(Player *player, Player *opponent)
{
    if (checkAvailable(player, 4))
    {
        ...
        int r = 0;               //  nb of hits
        int s[4] = {0, 0, 0, 0}; // to track the names of ships that have been sunk, for i [0->SHIPS_COUNT]: if (s[i] = i + 2), then getShipName(i+2) has been sunk
