...
int main() {
    ...
    // Clear screen for secrecy after names are entered
    system("cls"); 
...
void playerTurn(Player *attacker, Player *defender) {
    ...
    switch (action) {
        ...
        default:
            ...
            playerTurn(attacker, defender); // Retry the turn
            break;
    }

    displayGrid(defender); // Show the defender's grid (hidden information)
}

int fire(Player *attacker, Player *defender, int row, int col) {
    if (defender->grid[row][col] > 1) { // Check if the cell has a ship
...
void radarSweep(Player *player, int row, int col) {
    if (player->grid[row][col] > 1) {
        printf("Radar sweep detected a ship at %c%d!\n", col + 'A', row + 1); // Displaying the position
    } else {
        printf("Radar sweep found no ships at %c%d.\n", col + 'A', row + 1);
    }
}

void smokeScreen(Player *player, int row, int col) {
    printf("Smoke screen deployed at %c%d.\n", col + 'A', row + 1);
    // More effects can be implemented as needed
}

void checkGameOver(Player *player1, Player *player2) {
    int sunkShips1 = 0, sunkShips2 = 0;

    // Count sunk ships for player1
    for (int i = 2; i <= 5; i++) {
        if (i == player1->grid[0][0]) { // Checking for sunk ships (You may change this logic)
            sunkShips1++;
        }
    }

    // Count sunk ships for player2
    for (int i = 2; i <= 5; i++) {
        if (i == player2->grid[0][0]) { // Checking for sunk ships (You may change this logic)
            sunkShips2++;
        }
    }

    if (sunkShips1 == 4) { // 4 ships are sunk for player1
        printf("%s wins!\n", player2->name);
        exit(0);
    }
    if (sunkShips2 == 4) { // 4 ships are sunk for player2
        printf("%s wins!\n", player1->name);
        exit(0);
    }
}
...
void clearGrid(Player *player) {
    ...
    free(player->grid); // Free the grid itself
}
